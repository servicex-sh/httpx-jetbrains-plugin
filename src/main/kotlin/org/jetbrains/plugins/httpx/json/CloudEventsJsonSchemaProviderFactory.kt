...
class CloudEventsJsonSchemaProviderFactory : ContentAwareJsonSchemaFileProvider {
    override fun getSchemaFile(psiFile: PsiFile): VirtualFile? {
        if (psiFile is JsonFile) {
            ...
            if (psiLanguageInjectionHost is HttpMessageBody) {
                ...
                if (headerField != null && headerField.headerFieldValue?.text == "application/cloudevents+json") {
                    return VfsUtil.findFileByURL(this.javaClass.getResource("/cloudevents-schema.json")!!)
                }
            }
        }
        return null
